...
func Select(sqlDbOrTx interface{}, f sqlmapper.Filter) ([]DocClassEntity, error) {

	const semLogContext = "docclass::select"
...
func SelectByPrimaryKey(sqlDbOrTx interface{}, pk PrimaryKey) (DocClassEntity, error) {

	const semLogContext = "docclass::select-by-primary-key"
	...
	case *sqlx.Tx:
		err = db.Get(&ent, sqlStmt.GetStatement(), sqlStmt.GetParams()...)
	default:
		return DocClassEntity{}, fmt.Errorf("select accepts *sqlx.DB or *sqlx.Tx objects, provided %T", sqlDbOrTx)
	}
