...
import (
	...
	"github.com/GPA-Gruppo-Progetti-Avanzati-SRL/tpm-cpx-docclass/docclass/pqstore/docclass"
...
func TestDocClassEntity(t *testing.T) {
	...
	if doDDL {
		t.Log("ddl execution")
		sqlDb.MustExec(docclass.DocClassEntityTableDDL)
		defer sqlDb.MustExec(docclass.DocClassEntityTableDropDDL)
	}

	t.Log("select statement")
	f := sqlmapper.NewFilterBuilder().Limit(2).Offset(0)
	l, err := docclass.Select(sqlDb, f.Build())
	require.NoError(t, err)
	for i, e := range l {
		t.Logf("row: %d - %v", i, e)
	}
	t.Log("select-by-primary-key statement")
	e, err := docclass.SelectByPrimaryKey(sqlDb, docclass.PrimaryKey{Id: docclass.MustToMax30Text("pt_contratti")})
	require.NoError(t, err)
	t.Log(e)
